...
export default function Results() {
  ...
  const stats = useMemo(() => {
    ...
    const positive = resultsData.results.filter(r => r.sentiment === "Positive").length;
    const negative = resultsData.results.filter(r => r.sentiment === "Negative").length;
    const neutral = resultsData.results.filter(r => r.sentiment === "Neutral").length;
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <div className="text-start">
        ...
        <p className="text-sm text-gray-600 mt-1">Analysis of your social media post and comments</p>
      </div>
      ...
      {activeTab === 'overview' ? (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white p-4 rounded-xl border">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-gray-600">Main Post</span>
              </div>
          ...
          <div className="bg-white p-4 rounded-xl border">
            ...
            <div className="space-y-3">
              {Object.entries(stats.sentimentDistribution).map(([sentiment, percentage]) => (
              ...
              <div className="flex items-start gap-3 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className={`text-sm font-medium ${getSentimentColor(resultsData.results[0].score.positive)}`}>
                      Sentiment Score: {Math.round(resultsData.results[0].score.positive * 100)}%
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-4">
          {resultsData.results.slice(1).map((result, index) => (
            <div key={index} className="bg-white p-4 rounded-xl border space-y-3">
              <div className="flex items-start gap-3">
                {getSentimentIcon(result.sentiment)}
                <div className="flex-1">
                  <p className="text-gray-800">{result.content}</p>
                  <div className="mt-2">
                    <span className={`text-sm font-medium ${getSentimentColor(result.score.positive)}`}>
